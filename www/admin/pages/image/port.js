var md = angular.module('module_image/port', ['base']);
md.controller('MainCtrl', function ($scope, $http, uiTips, uiValid) {
    ...
    var old = store.get('image_port_keyword');
    if (old && old.keyword) {
        $scope.tmp.keyword = old.keyword;
    }

    $scope.queryLl = function (pageNum) {
        var p = {pageNum: pageNum || 1, keyword: $scope.tmp.keyword};
        if (p.keyword) {
            store.set('image_port_keyword', p);
        } else {
            store.remove('image_port_keyword');
        }

        $http.get('/dms/image/config/port/list', {params: p}).success(function (data) {
            $scope.ll = data.list;
    ...
    $scope.save = function () {
        ...
        $http.post('/dms/image/config/port/update', one).success(function (data) {
            if (data.id) {
	...
	$scope.delete = function(one){
		uiTips.confirm('Sure Delete - ' + one.name + '?', function(){
			$http.delete('/dms/image/config/port/delete/' + one.id).success(function(data){
				if(data.flag){
