...
class User {
    ...
    private boolean isAccess(PermitType permitType, int id = 0) {
        ...
        if (permitType == PermitType.namespace) {
            def namespaceOne = new NamespaceDTO(id: id).queryFields('cluster_id').one() as NamespaceDTO
            if (permitList.any {
            ...
            def appOne = new AppDTO(id: id).queryFields('cluster_id,namespace_id').one() as AppDTO
            if (permitList.any {
